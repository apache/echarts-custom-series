  ...
  (window as any).setImmediate = (function setupSetImmediate() {
    return (
      ...
      (function setupSetZeroTimeout() {
        ...
        var callbacks: (Function | undefined)[] = [undefined];
        var message = 'zero-timeout-message';

        // Like setTimeout, but only takes a function argument.  There's
        // no time argument (always zero) and no arguments (you have to
        // use a closure).
        var setZeroTimeout = function setZeroTimeout(callback: Function) {
...
var getItemExtraData = function (item: any) {
...
var shuffleArray = function shuffleArray(arr: any[]) {
...
var WordCloud = function WordCloud(elements: any, options: any) {
  ...
  if (typeof settings.shape !== 'function') {
    switch (settings.shape) {
      ...
      case 'diamond':
        ...
        // +0+..+2*PI
        settings.shape = function shapeSquare(theta: number) {
      ...
      case 'triangle-forward':
        ...
        settings.shape = function shapeTriangle(theta: number) {
      ...
      case 'pentagon':
        settings.shape = function shapePentagon(theta: number) {
      ...
      case 'star':
        settings.shape = function shapeStar(theta: number) {
  ...
  var canFitText = function canFitText(
    gx: number,
    gy: number,
    gw: number,
    gh: number,
    ...
    elements.forEach(function (el: any) {
      if (el.getContext) {
        ...
        for (var cssProp in styleRules) {
          span.style[cssProp as any] = styleRules[cssProp];
        }
        if (attributes) {
...
(WordCloud as any).isSupported = isSupported;
(WordCloud as any).minFontSize = minFontSize;
