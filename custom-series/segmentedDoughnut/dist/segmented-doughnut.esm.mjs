...
import { number, zrUtil, format } from 'echarts';
...
var renderItem = function (params, api) {
    ...
    var cx = number.parsePercent(center[0], api.getWidth());
    var cy = number.parsePercent(center[1], api.getHeight());
    var r = number.parsePercent(radius[1], size / 2);
    var r0 = number.parsePercent(radius[0], size / 2);
    var padAngle = ((zrUtil.retrieve2(itemPayload.padAngle, 2) || 0) * Math.PI) / 180;
    ...
    var labelEl;
    if (label && label.show) {
        var text = format.formatTpl(label.formatter || '{c}/{b}', {
